...
class JSONFormatter:
    ...
    def __init__(
        ...
        if fields is None:
            ...
            try:
                ...
                raise TypeError(
                    "A fields argument must be either None, an iterable or a"
                    " str."
                )
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # NOTE: it is better to pass args separately in the logger
        # - logger.info('Hello {}', 'world!') than using str.format()
        # or f-strings since the message will be formed
        # only if the logging level is appropriate
        # while strings will be interpolated before logger checks
        # the message's level wasting program time

        # mypy error when assigning to record.message:
        # Type cannot be declared in assignment to non-self attribute
        message: str = (
            record.msg.format(*record.args) if record.args else record.msg
        )
        ...
        # fields provided with the use of extra dict
        # are available in record.__dict__
        selection: list[str] = [
            attr for attr in record.__dict__ if attr in self.fields
        ]
        ...
        log: dict[str, Any] = {
            attr: getattr(record, attr) for attr in selection
        }

        return json.dumps(
            log,
            indent=self.indent,
            # to avoid printing utf codes for non-ascii characters
            ensure_ascii=False,
        )
